...
def _build_css(bg_hex: str, rose_gold_hex: str) -> str:
    """Generate the CSS block for the logo page."""
    return f'''<style>
...
def _build_js_helpers() -> str:
    """Generate the reusable Three.js helper functions."""
    return '''// ═══ HELPERS ═══
...
def _build_js_background(bg_hex: str) -> str:
...
def _build_js_scene(
    palette: dict, layout: dict, animations: dict,
    shimmer_cfg: dict, particle_cfg: dict,
    node_glow_scale: float, node_bloom_scale: float,
    node_glow_opacity: float, node_bloom_opacity: float,
    arc_runner_radius: float,
) -> str:
    """Generate the Three.js scene setup and logo object creation JS."""
    edge_thickness = layout.get('edge_thickness', palette['sizing']['edge_thickness'])

    return f'''// ═══ THREE.JS SETUP ═══
...
for(const [a,b,etype] of EDGES) {{
  ...
  const line=createLine(x1,y1,x2,y2, {edge_thickness}, PAL.copper, op);
  line.position.z=0.2;
  logoGroup.add(line);
}}
...
def _build_js_animation(shimmer_cfg: dict) -> str:
    """Generate the BFS wave and animation loop JS."""
    return f'''// ═══ BFS WAVE ═══
let waveTimer=2.0;
...
def _build_html(
    ...
    js_data = "\n".join([
        _color_map_js(palette),
        _nodes_js(nodes),
        _edges_js(edges),
        _arcs_js(arcs),
        _nib_js(nib),
        _runner_paths_js(runners),
        _ring_js(layout["ring"]),
        _anim_js(animations),
        _adjacency_js(animations),
        _nebula_js(palette),
        f"const STAR_COUNT = {star_count};",
        f"const CENTER_OFFSET_Y = {co['y']};",
    ])

    css = _build_css(bg_hex, _color_hex("rose_gold"))
    js_helpers = _build_js_helpers()
    js_bg = _build_js_background(bg_hex)
    js_scene = _build_js_scene(
        palette, layout, animations, shimmer_cfg, particle_cfg,
        node_glow_scale, node_bloom_scale, node_glow_opacity,
        node_bloom_opacity, arc_runner_radius,
    )
    js_anim = _build_js_animation(shimmer_cfg)
